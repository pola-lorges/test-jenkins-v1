...
test.beforeEach( async ({ page }) => {
  ...
  test('Recherche produit failed @failed', async ({ page }) => { 
    ...
    await page.getByPlaceholder('Rechecher un produit').fill(product_data.invalid_product);

    //point de verification
    await expect(page.getByPlaceholder('Rechecher un produit')).toHaveValue(product_data.invalid_product);

    await page.locator('#style_content_input_wrapper__eNPFq svg').click();

    //verification de la reponse
    await expect(page.getByText(product_data.product_not_found)).toHaveText(product_data.product_not_found);
});